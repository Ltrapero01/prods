...
SELECT invoice_num FROM invoice WHERE cust_id = '1619' AND invoice_date = '13-SEP-2007';

-- 5.	Display the id and the name for all customers whose sales representative has an id of either 237 or 268.
SELECT cust_id, cust_name FROM customer WHERE rep_id IN ('237','268');
...
SELECT MIN(cust_balance) AS min_balance FROM customer;
...
SELECT rep_id, SUM(cust_balance) AS total_balance FROM customer GROUP BY rep_id ORDER BY rep_id;
...
SELECT rep_id, SUM(cust_balance) AS total_balance FROM customer GROUP BY rep_id HAVING SUM(cust_balance) > 12000;
